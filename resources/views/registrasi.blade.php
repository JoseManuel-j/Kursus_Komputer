    ...
    <div class="container py-5">
        <div class="register-box shadow mx-auto" style="max-width: 800px;">
            ...
            <form action="/pendaftaran" method="POST" enctype="multipart/form-data">
                ...
                <div id="form_cicilan" class="mb-4" style="display:none;">
                    <div class="payment-box">
                        <label class="form-label fw-bold">Nominal Cicilan (Tiap Termin):</label>
                        <div class="row">
                            @for($i=1; $i<=3; $i++)
                                <div class="col-md-4 mb-2">
                                    <input type="text" name="cicilan[]" class="form-control cicilan-input"
                                           value="{{ old('cicilan.'.($i-1)) }}"
                                           placeholder="Angsuran {{ $i }}" onkeyup="formatRupiah(this); hitungSisaCicilan();">
                                </div>
                            @endfor
                        </div>
                        <div id="info_sisa_cicilan" class="mt-2 fw-bold"></div>
                        @error('cicilan')
                            <div class="text-danger fw-bold mt-2">{{ $message }}</div>
                        @enderror
                        <small class="text-muted">*Nominal disarankan otomatis, boleh diubah, tapi total harus sama persis dengan tagihan.</small>
                    </div>
                </div>

                <!--C. Rincian & Metode Pembayaran -->
                ...
                <div class="payment-box">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-5 mb-3 mb-md-0 border-end">
                            ...
                            <p class="mb-1 fw-bold fs-5">6090012451</p>
                            <p class="mb-0">A/N LPK Phitagoras</p>
                        </div>
                        <div class="col-md-7 text-center">
                            <h5 class="fw-bold mb-3">QRIS</h5>
                            <img src="{{ asset('images/qris-phitagoras.jpeg') }}" alt="QRIS LPK Phitagoras" class="img-fluid border p-2 rounded bg-white shadow-sm" style="max-width: 340px; width: 100%;">
                        </div>
                    </div>
                ...
                <div class="alert border-0 shadow-sm d-flex align-items-center mb-4 mt-4" style="border-radius: 15px; background-color: #eef2ff; color: #4338ca;">
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function getTotalTagihan() {
            ...
            const biayaKelas = opt ? (parseInt(opt.getAttribute("data-biaya")) || 0) : 0;
            return biayaKelas + biayaPendaftaran;
        }

        function bulatkanRibuan(angka) {
            return Math.round(angka / 1000) * 1000;
        }
        ...
        function suggestCicilan() {
            const total = getTotalTagihan();
            if (total <= 0) return;
            const base = bulatkanRibuan(total / 3);
            const sisaAkhir = total - (base * 2);
            cicilanInputs[0].value = base.toLocaleString("id-ID");
            cicilanInputs[1].value = base.toLocaleString("id-ID");
            cicilanInputs[2].value = sisaAkhir.toLocaleString("id-ID");
            hitungSisaCicilan();
        }

        window.hitungSisaCicilan = function () {
            const total = getTotalTagihan();
            let jumlah = 0;
            cicilanInputs.forEach(input => {
                jumlah += parseInt(input.value.replace(/\D/g, '')) || 0;
            });
            const selisih = total - jumlah;

            if (selisih === 0) {
                infoSisa.className = "mt-2 fw-bold text-success";
                infoSisa.innerText = "✓ Total cicilan sudah sesuai tagihan.";
                submitBtn.disabled = false;
            } else if (selisih > 0) {
                infoSisa.className = "mt-2 fw-bold text-danger";
                infoSisa.innerText = "Kurang Rp " + selisih.toLocaleString("id-ID") + ", silakan alokasikan ke termin lain.";
                submitBtn.disabled = true;
            } else {
                infoSisa.className = "mt-2 fw-bold text-danger";
                infoSisa.innerText = "Kelebihan Rp " + Math.abs(selisih).toLocaleString("id-ID") + ", kurangi salah satu termin.";
                submitBtn.disabled = true;
            }
        };

        window.toggleCicilan = function () {
            const tipe = tipePembayaran.value;
            document.getElementById("form_cicilan").style.display = (tipe === "angsuran") ? "block" : "none";
            submitBtn.disabled = false;
            if (tipe === "angsuran") suggestCicilan();
        };
