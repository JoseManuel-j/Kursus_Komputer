...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: Arial, sans-serif; background: #f4f7fc; color: #172a45; 
        a { text-decoration: none; }

        /* =========================
           TOPBAR (ala detik.com)
        ========================= */
        .topbar { background: #172a45; color: #cbd5e1; font-size: 13px; padding: 6px 0; }
        .topbar .container { display: flex; justify-content: space-between; align-items: center; }
        .topbar-right a { color: #cbd5e1; margin-left: 14px; font-size: 15px; }
        .topbar-right a:hover { color: #ffb800; }

        /* =========================
           NAVBAR (sticky, ala detik.com)
        ========================= */
        .navbar-main { background: #ffffff; box-shadow: 0 2px 10px rgba(15, 23, 42, 0.08); position: sticky; top: 0; z-index: 999; padding: 14px 0; }
        .navbar-main .container { display: flex; align-items: center; justify-content: space-between; }
        .brand-logo { font-size: 24px; font-weight: 800; color: #172a45; display: flex; align-items: center; }
        .brand-logo-img { height: 48px; width: auto; display: block; }
        .nav-menu { display: flex; gap: 28px; list-style: none; }
        .nav-menu a { color: #172a45; font-weight: 700; font-size: 14px; letter-spacing: 0.4px; text-transform: uppercase; padding-bottom: 6px; border-bottom: 2px solid transparent; transition: 0.2s; }
        .nav-menu a:hover, .nav-menu a.active { color: #e5a600; border-bottom: 2px solid #ffb800; }
        .nav-auth a { font-weight: 700; font-size: 14px; margin-left: 10px; padding: 8px 18px; border-radius: 6px; }
        .nav-auth .btn-login { color: #172a45; border: 1px solid #172a45; }
        .nav-auth .btn-login:hover { background: #172a45; color: #fff; }
        .nav-auth .btn-register { background: #ffb800; color: #172a45; }
        .nav-auth .btn-register:hover { background: #e5a600; }

        /* =========================
           GALERI KEGIATAN
        ========================= */
        .galeri-section { background: #0f172a; padding: 40px 0 55px; }
        .galeri-label { color: #ffb800; font-size: 13px; font-weight: 800; letter-spacing: 2px; margin-bottom: 8px; display: block; }
        .galeri-title { color: #fff; font-size: 28px; font-weight: 800; margin-bottom: 20px; }
        .slider-wrap { position: relative; border-radius: 14px; overflow: hidden; background: #1e293b; box-shadow: 0 15px 40px rgba(0,0,0,0.35); }
        ...
        .slide.img-error img { display: none; }
        ...
        .slide-fallback-icon { position: absolute; inset: 0; display: none; align-items: center; justify-content: center; font-size: 90px; color: rgba(255,255,255,0.25); }
        .slide.img-error .slide-fallback-icon { display: flex; }
        .slide-caption { position: relative; z-index: 2; width: 100%; padding: 30px 90px 24px 30px; background: linear-gradient(to top, rgba(0,0,0,0.85), rgba(0,0,0,0)); color: #fff; }
        .slide-caption .tag { display: inline-block; background: #ffb800; color: #172a45; font-size: 11px; font-weight: 800; letter-spacing: 1px; padding: 3px 10px; border-radius: 4px; margin-bottom: 10px; text-transform: uppercase; }
        .slide-caption h3 { font-size: 22px; font-weight: 700; margin-bottom: 6px; }
        .slide-caption p { font-size: 14px; color: #cbd5e1; }
        .slide-counter { position: absolute; top: 18px; right: 20px; z-index: 3; background: rgba(0,0,0,0.55); color: #fff; font-size: 13px; font-weight: 700; padding: 5px 12px; border-radius: 20px; }
        .slide-nav-btn { position: absolute; top: 50%; transform: translateY(-50%); z-index: 4; background: rgba(255,255,255,0.9); color: #172a45; border: none; width: 42px; height: 42px; border-radius: 50%; font-size: 18px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s; }
        .slide-nav-btn:hover { background: #ffb800; }
        ...
        .thumb-item { flex: 0 0 auto; width: 130px; height: 74px; border-radius: 8px; overflow: hidden; position: relative; cursor: pointer; opacity: 0.55; border: 2px solid transparent; transition: 0.2s; }
        ...
        .thumb-item.active, .thumb-item:hover { opacity: 1; border-color: #ffb800; }
        .thumb-item span { position: absolute; bottom: 4px; left: 6px; right: 6px; color: #fff; font-size: 11px; font-weight: 700; text-shadow: 0 1px 3px rgba(0,0,0,0.8); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

        /* =========================
           LAIN-LAIN
        ========================= */
        .intro-section { padding: 55px 0 20px; text-align: center; }
        .intro-section h1 { font-size: 38px; font-weight: 800; color: #172a45; margin-bottom: 14px; }
        .intro-section p { color: #64748b; font-size: 16px; max-width: 600px; margin: 0 auto 22px; line-height: 1.7; }
        .program-section { padding: 70px 0 100px; background: #f8fafc; }
        .program-header { text-align: center; max-width: 700px; margin: 0 auto 55px; }
        .program-label { display: inline-block; color: #f5a800; font-size: 14px; font-weight: 700; letter-spacing: 2px; margin-bottom: 12px; }
        .program-header h2 { font-size: 42px; font-weight: 700; color: #172a45; margin-bottom: 15px; }
        .program-header p { color: #64748b; font-size: 17px; line-height: 1.7; }
        .program-card { height: 100%; background: white; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 35px rgba(15, 23, 42, 0.08); transition: transform 0.3s ease, box-shadow 0.3s ease; }
        .program-card:hover { transform: translateY(-8px); box-shadow: 0 20px 45px rgba(15, 23, 42, 0.15); }
        .program-banner { width: 100%; padding: 22px 30px; color: #fff; display: flex; align-items: center; gap: 14px; }
        .program-banner i { font-size: 26px; }
        .program-banner span { font-weight: 700; font-size: 15px; }
        .banner-paket { background: linear-gradient(120deg, #1d4ed8, #172a45); }
        .banner-satuan { background: linear-gradient(120deg, #f5a800, #b8860b); }
        .program-content { padding: 30px; }
        .program-content h3 { font-size: 28px; font-weight: 700; margin-bottom: 12px; color: #172a45; }
        .program-content p { color: #64748b; line-height: 1.7; margin-bottom: 15px; }
        .program-teaser-link { display: block; height: 100%; }
        .program-teaser { height: 100%; background: white; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 35px rgba(15, 23, 42, 0.08); transition: transform 0.3s ease, box-shadow 0.3s ease; }
        .program-teaser:hover { transform: translateY(-8px); box-shadow: 0 20px 45px rgba(15, 23, 42, 0.15); }
        .program-teaser-content { padding: 30px; }
        .program-teaser-content h3 { font-size: 26px; font-weight: 700; margin-bottom: 12px; color: #172a45; }
        .program-teaser-content p { color: #64748b; line-height: 1.7; margin-bottom: 18px; }
        .teaser-cta { color: #172a45; font-weight: 700; font-size: 14px; }
        .teaser-cta i { margin-left: 4px; transition: 0.2s; }
        .program-teaser-link:hover .teaser-cta i { transform: translateX(4px); }
        .program-button { display: inline-block; padding: 12px 25px; background: #ffb800; color: #172a45; text-decoration: none; font-weight: 700; border-radius: 8px; transition: 0.3s; }
        .program-button:hover { background: #e5a600; color: #172a45; transform: translateY(-2px); }
        .popular-section { padding: 100px 0; background: white; }
        .popular-title { font-size: 42px; font-weight: 700; }
        .card-course { border: none; border-radius: 20px; transition: 0.3s; height: 100%; }
        .card-course:hover { transform: translateY(-10px); }
        .site-footer { background: #0f172a; color: #cbd5e1; padding: 50px 0 20px; }
        .site-footer h5 { color: #fff; font-size: 16px; font-weight: 700; margin-bottom: 14px; }
        .site-footer h5 i { color: #ffb800; margin-right: 6px; }
        .footer-text { font-size: 14px; line-height: 1.8; color: #94a3b8; }
        .footer-wa { display: inline-flex; align-items: center; gap: 8px; margin-top: 10px; color: #25d366; font-weight: 700; font-size: 15px; }
        .footer-wa:hover { color: #1ebc59; }
        .footer-divider { border-color: rgba(255,255,255,0.1); margin: 30px 0 18px; }
        .footer-bottom { font-size: 13px; color: #64748b; padding-bottom: 10px; }
...
<a href="https://www.instagram.com/phitagoras.site/" target="_blank" title="Instagram">
    <i class="fab fa-instagram"></i>
</a>

<a href="https://wa.me/6285217184694" target="_blank" title="WhatsApp">
    ...
    <div class="container">
        ...
        <div class="slider-wrap">
            ...
            <div class="slider-track" id="sliderTrack">
                ...
                <!-- 8 SLIDE STATIC BAWAAN TEMPLATE -->
                <div class="slide" data-fallback="1">
                    <img src="{{ asset('Images/kegiatan/kegiatan-1.jpg') }}" alt="Pelatihan Komputer Pegawai Kecamatan Ciputat" onerror="this.parentElement.classList.add('img-error')">
                ...
                <div class="slide" data-fallback="2">
                    <img src="{{ asset('Images/kegiatan/kegiatan-2.jpg') }}" alt="Praktik Komputer Pegawai Kecamatan Ciputat" onerror="this.parentElement.classList.add('img-error')">
                ...
                <div class="slide" data-fallback="3">
                    <img src="{{ asset('Images/kegiatan/kegiatan-3.jpg') }}" alt="Pelatihan Komputer Ibu PKK Rempoa" onerror="this.parentElement.classList.add('img-error')">
                ...
                <div class="slide" data-fallback="4">
                    <img src="{{ asset('Images/kegiatan/kegiatan-4.jpg') }}" alt="Praktik Microsoft Office Ibu PKK Rempoa" onerror="this.parentElement.classList.add('img-error')">
                ...
                <div class="slide" data-fallback="5">
                    <img src="{{ asset('Images/kegiatan/kegiatan-5.jpg') }}" alt="Pengarahan Sebelum Pelatihan Kader PKK Rengas" onerror="this.parentElement.classList.add('img-error')">
                ...
                <div class="slide" data-fallback="6">
                    <img src="{{ asset('Images/kegiatan/kegiatan-6.jpg') }}" alt="Pelatihan Komputer Kader PKK Rengas" onerror="this.parentElement.classList.add('img-error')">
                ...
                <div class="slide" data-fallback="7">
                    <img src="{{ asset('Images/kegiatan/kegiatan-7.jpg') }}" alt="Pelatihan Kader PKK Kecamatan Ciputat Timur" onerror="this.parentElement.classList.add('img-error')">
                ...
                <div class="slide" data-fallback="8">
                    <img src="{{ asset('Images/kegiatan/kegiatan-8.jpg') }}" alt="Suasana Pelatihan Pegawai Kecamatan Ciputat" onerror="this.parentElement.classList.add('img-error')">
                ...
                <!-- SLIDE DINAMIS DARI DATABASE (Nambah di urutan ke-9 dst) -->
                @foreach($galeri as $key => $foto)
                    @php $indexSlide = $key + 8; @endphp
                    <div class="slide" data-fallback="{{ ($indexSlide % 5) + 1 }}">
                        <img src="{{ asset('Images/kegiatan/' . $foto->nama_file) }}" alt="{{ $foto->judul }}" onerror="this.parentElement.classList.add('img-error')">
                        <div class="slide-fallback-icon"><i class="fa fa-image"></i></div>
                        <div class="slide-caption">
                            <span class="tag">Info Kegiatan</span>
                            <h3>{{ $foto->judul }}</h3>
                            <p>{{ $foto->keterangan ?? 'Dokumentasi kegiatan LPK Phitagoras.' }}</p>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="thumb-strip" id="thumbStrip">
            ...
            <!-- THUMBNAIL DINAMIS DARI DATABASE -->
            @foreach($galeri as $key => $foto)
                @php $indexSlide = $key + 8; @endphp
                <div class="thumb-item" data-fallback="{{ ($indexSlide % 5) + 1 }}" onclick="goToSlide({{ $indexSlide }})">
                    <img src="{{ asset('Images/kegiatan/' . $foto->nama_file) }}" alt="{{ $foto->judul }}" onerror="this.parentElement.classList.add('img-error')">
                    <span>{{ \Illuminate\Support\Str::limit($foto->judul, 15) }}</span>
                </div>
            @endforeach
...
<script>
    ...
    document.getElementById('topbar-date').textContent = hariIni.toLocaleDateString('id-ID', opsiTanggal);
    ...
    totalEl.textContent = totalSlides > 0 ? totalSlides : 1;
